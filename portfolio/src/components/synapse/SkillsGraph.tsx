...
export default function SkillsGraph() {
  ...
  useEffect(() => {
    ...
    // Build nodes with cluster centers
    const centers = [
      { x: W * 0.25, y: H * 0.3 },
      { x: W * 0.65, y: H * 0.28 },
      { x: W * 0.25, y: H * 0.72 },
    ...
    const draw = () => {
      ...
      nodes.forEach((n) => {
        ...
        const isConnected = hov ? nodes.find(x => x.id === hov)?.connections.includes(nodes.indexOf(n)) : false;
  ...
  return (
    <div className="relative w-full" style={{ height: 420 }}>
      <canvas
        ...
        role="img"
        aria-label="Skills graph showing proficiency nodes grouped by category"
      />
      {/* Category legend */}
